...
def onelayer(X, Y, layersize=10):
    ...
    w = tf.Variable(tf.zeros([shape, layersize]))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    shape = X.get_shape().as_list()[1]
    w1 = tf.Variable(tf.random_normal([shape, hiddensize], stddev = 0.1))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    conv1 = tf.layers.conv2d(inputs = X, filters = convlayer_sizes[0], \
        kernel_size = filter_shape, padding = "same", activation = tf.nn.relu)
    conv2 = tf.layers.conv2d(inputs = conv1, filters = convlayer_sizes[1], \
        kernel_size = filter_shape, padding = "same", activation = tf.nn.relu)
    shape0 = conv2.get_shape().as_list()[1]
    shape1 = conv2.get_shape().as_list()[2]
    num_units = shape0 * shape1 * convlayer_sizes[1]
    flattened = tf.reshape(conv2, [-1,num_units])
    w = tf.Variable(tf.random_normal([num_units, convlayer_sizes[1]], stddev = 0.1))
    b = tf.Variable(tf.zeros([convlayer_sizes[1]]))
