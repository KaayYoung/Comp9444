...
DIS_INTERVAL = 20 # number of intervals if the action is continuous
BOTTOM_LIMIT = -2 # the lowest limit for action
TOP_LIMIT = 2 # the highest limit for action
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    w1 = tf.get_variable("w1", [state_dim, hidden_nodes])
    b1 = tf.get_variable("b1", [hidden_nodes])
    w2 = tf.get_variable("w2", [hidden_nodes, action_dim])
    ...
    # Target network
    w3 = tf.get_variable("w3", [state_dim, hidden_nodes])
    b3 = tf.get_variable("b3", [hidden_nodes])
    w4 = tf.get_variable("w4", [hidden_nodes, action_dim])
    b4 = tf.get_variable("b4", [action_dim])
    h_layer2 = tf.nn.relu(tf.matmul(state_in, w3) + b3)

    q_next = tf.matmul(h_layer2, w4) + b4
...
def get_env_action(action):
    ...
    """
    if is_continuous == True:
        # action_dis for saving discretised action spaces, with 20 intervals
        action_dis = BOTTOM_LIMIT + action * (TOP_LIMIT - BOTTOM_LIMIT) / 20
        return action_dis
    else:
        return action
    ...
    for i in range(0, BATCH_SIZE):
        ...
        if sample_is_done:
            ...
            if is_continuous == False:
                max_next = np.argmax(Q_value_batch[i])
                target_val = reward_batch[i] + GAMMA * Q_next[i, max_next]
            if is_continuous == True or run_dqn == True:
...
def setup():
    global BOTTOM_LIMIT, TOP_LIMIT, is_continuous, run_dqn
    ...
    if  env_name == "Pendulum-v0":
        is_continuous = True
        BOTTOM_LIMIT = env.action_space.low[0]
        TOP_LIMIT = env.action_space.high[0]
